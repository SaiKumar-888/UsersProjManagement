...
export const createProject = catchAsync(async (req: Request, res: Response) => {
  ...
  res.status(201).json(
    new ApiResponse("Project created successfully", project)
  );
});
...
export const updateProject = catchAsync(async (req: Request, res: Response) => {
  ...
  res.status(200).json(
    new ApiResponse("Project updated successfully", updatedProject)
  );
});
...
export const deleteProject = catchAsync(async (req: Request, res: Response) => {
  ...
  res.status(200).json(
    new ApiResponse(result.message, null)
  );
});

export const addProjectMemberController = catchAsync(async (req: Request, res: Response) => {
  const projectId = getRequiredParam(req.params.id, "project id");
  const member = await addProjectMemberService(req.user!.id, projectId, req.body.userId, req.body.role);

  res.status(201).json(
    new ApiResponse("Project member added successfully", member)
  );
});